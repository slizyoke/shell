    ...
    if(isset($_POST['link'])){
        if(file_get_contents($_POST['link'])){
            ...
            foreach(get_data($_POST['link']) as $info){
                ...
                if($co){
                    echo "Trinity~ Username (<font color=red>$info[0]</font>) Password (<font color=red>$info[1]</font>)<br />";
                    mysql_close($co);   
                }
            }
        }else{
    ...
    if(isset($_POST['usernames']) && isset($_POST['passwords'])){
        ...
        foreach(explode("\n",$usernames) as $username){
            ...
            foreach(explode("\n",$passwords) as $password){
                ...
                if($co){
                    mysql_close($co);
                    echo "Trinity~ Username (<font color=red>$username</font>) Password (<font color=red>$password</font>)<br />";
                    $count++;
                }
            }
        }
